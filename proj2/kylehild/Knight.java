...
public class Knight extends ChessPiece {
    ...
    public int getNumberOfMoves(final ChessBoard board, final boolean printMoves){
        ...
        if(printMoves) System.out.println(color + "N at " + getColChar(col) + (row+1) + " valid moves:");

        numMoves += CheckBoardAt(board, row+2, col+1, printMoves);
        numMoves += CheckBoardAt(board, row+1, col+2, printMoves);
        numMoves += CheckBoardAt(board, row-1, col+2, printMoves);
        numMoves += CheckBoardAt(board, row-2, col+1, printMoves);
        numMoves += CheckBoardAt(board, row-2, col-1, printMoves);
        numMoves += CheckBoardAt(board, row-1, col-2, printMoves);
